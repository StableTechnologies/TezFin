...
class Contract(sp.Contract):
  def __init__(self):
    ...
    self.init(accrualBlockNumber = 0,
              ...
              administrator = sp.address('KT1A7VB84jAoQyqBxy5rghfvDkPaQi6XRTyp'),
              balances = {},
              borrowIndex = 1000000000000000000,
              borrowRateMaxMantissa = 5000000000000,
              borrowRatePerBlock = 0,
              comptroller = sp.address('KT1AD8rvgSHoM8SrNRzCVxLpDM6eKFgNH3kU'),
              expScale = 1000000000000000000,
              halfExpScale = 500000000000000000,
              initialExchangeRateMantissa = 1000000000000000000,
              interestRateModel = sp.address('KT1NRFY1vtLGh2buux35uQpabyJRsqYjSAVB'),
  ...
  def addReservesInternal(self, params):
    ...
    compute_CToken_734 = sp.local("compute_CToken_734", sp.as_nat(sp.level - self.data.accrualBlockNumber, message = 'SUBTRACTION_UNDERFLOW'))
    sp.verify(compute_CToken_734.value < self.data.accrualIntPeriodRelevance, 'CT_INTEREST_OLD')
  ...
  def borrow(self, params):
    ...
    sp.set_type(sp.level, sp.TNat)
    sp.set_type(self.data.accrualBlockNumber, sp.TNat)
    compute_CToken_734i = sp.local("compute_CToken_734i", sp.as_nat(sp.level - self.data.accrualBlockNumber, message = 'SUBTRACTION_UNDERFLOW'))
    sp.verify(compute_CToken_734i.value < self.data.accrualIntPeriodRelevance, 'CT_INTEREST_OLD')
    sp.verify(self.data.isAccrualInterestValid, 'CT_INTEREST_INVALID')
    sp.if False:
  ...
  def doAccrueInterest(self, params):
    ...
    compute_CToken_549 = sp.local("compute_CToken_549", sp.record(mantissa = params * sp.as_nat(sp.level - self.data.accrualBlockNumber)))
    sp.set_type(compute_CToken_549.value, sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
    sp.set_type(compute_CToken_549.value, sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
    sp.set_type(self.data.totalBorrows, sp.TNat)
    sp.set_type(compute_CToken_549.value.mantissa * self.data.totalBorrows, sp.TNat)
    sp.set_type(sp.record(mantissa = compute_CToken_549.value.mantissa * self.data.totalBorrows), sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
    compute_CToken_550 = sp.local("compute_CToken_550", (compute_CToken_549.value.mantissa * self.data.totalBorrows) // self.data.expScale)
    self.data.totalBorrows = compute_CToken_550.value + self.data.totalBorrows
    sp.set_type(sp.record(mantissa = self.data.reserveFactorMantissa), sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
    sp.set_type(compute_CToken_550.value, sp.TNat)
    sp.set_type(self.data.totalReserves, sp.TNat)
    sp.set_type(sp.record(mantissa = self.data.reserveFactorMantissa), sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
    sp.set_type(sp.record(mantissa = self.data.reserveFactorMantissa), sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
    sp.set_type(compute_CToken_550.value, sp.TNat)
    sp.set_type(self.data.reserveFactorMantissa * compute_CToken_550.value, sp.TNat)
    sp.set_type(sp.record(mantissa = self.data.reserveFactorMantissa * compute_CToken_550.value), sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
    self.data.totalReserves = ((self.data.reserveFactorMantissa * compute_CToken_550.value) // self.data.expScale) + self.data.totalReserves
    sp.set_type(compute_CToken_549.value, sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
    ...
    sp.set_type(compute_CToken_549.value, sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
    sp.set_type(self.data.borrowIndex, sp.TNat)
    sp.set_type(compute_CToken_549.value.mantissa * self.data.borrowIndex, sp.TNat)
    sp.set_type(sp.record(mantissa = compute_CToken_549.value.mantissa * self.data.borrowIndex), sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
    self.data.borrowIndex = ((compute_CToken_549.value.mantissa * self.data.borrowIndex) // self.data.expScale) + self.data.borrowIndex
    self.data.accrualBlockNumber = sp.level
  ...
  def getAccountSnapshot(self, params):
    compute_CToken_355 = sp.local("compute_CToken_355", sp.record(account = sp.fst(params), borrowBalance = 0, cTokenBalance = 0, exchangeRateMantissa = 0))
    sp.if self.data.balances.contains(sp.fst(params)):
      ...
      sp.verify(compute_CToken_734.value < self.data.accrualIntPeriodRelevance, 'CT_INTEREST_OLD')
      sp.verify(self.data.isAccrualInterestValid, 'CT_INTEREST_INVALID')
      compute_CToken_355.value.cTokenBalance = self.data.balances[sp.fst(params)].balance
      borrowSnapshot = sp.local("borrowSnapshot", self.data.balances[sp.fst(params)].accountBorrows)
      borrowBalance = sp.local("borrowBalance", 0)
      sp.if borrowSnapshot.value.principal > 0:
        borrowBalance.value = (borrowSnapshot.value.principal * self.data.borrowIndex) // borrowSnapshot.value.interestIndex
      compute_CToken_355.value.borrowBalance = borrowBalance.value
      excRate = sp.local("excRate", self.data.initialExchangeRateMantissa)
      sp.if self.data.totalSupply > 0:
        sp.set_type(sp.fst(sp.ediv(sp.balance, sp.mutez(1)).open_some(message = sp.unit)), sp.TNat)
        sp.set_type(0, sp.TNat)
        ...
        sp.set_type((sp.as_nat((sp.as_nat(sp.fst(sp.ediv(sp.balance, sp.mutez(1)).open_some(message = sp.unit)) - 0, message = 'SUBTRACTION_UNDERFLOW') + self.data.totalBorrows) - self.data.totalReserves) * self.data.expScale) // self.data.totalSupply, sp.TNat)
        excRate.value = (sp.as_nat((sp.as_nat(sp.fst(sp.ediv(sp.balance, sp.mutez(1)).open_some(message = sp.unit)) - 0, message = 'SUBTRACTION_UNDERFLOW') + self.data.totalBorrows) - self.data.totalReserves) * self.data.expScale) // self.data.totalSupply
      compute_CToken_355.value.exchangeRateMantissa = excRate.value
    __s3 = sp.local("__s3", compute_CToken_355.value)
  ...
  def mint(self, params):
    ...
    sp.if True:
      ...
      compute_CToken_751 = sp.local("compute_CToken_751", (params * self.data.expScale) // excRate.value)
      amount.value = compute_CToken_751.value
    sp.else:
      sp.set_type(excRate.value, sp.TNat)
      sp.set_type(sp.record(mantissa = excRate.value), sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
      sp.set_type(sp.record(mantissa = excRate.value), sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
      ...
      compute_CToken_753 = sp.local("compute_CToken_753", (excRate.value * params) // self.data.expScale)
      amount.value = compute_CToken_753.value
