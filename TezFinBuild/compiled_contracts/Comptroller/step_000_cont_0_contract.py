...
class Contract(sp.Contract):
  def __init__(self):
    self.init_type(sp.TRecord(account_liquidity = sp.TBigMap(sp.TAddress, sp.TRecord(liquidity = sp.TInt, updateLevel = sp.TNat, valid = sp.TBool).layout(("liquidity", ("updateLevel", "valid")))), activeOperations = sp.TSet(sp.TNat), administrator = sp.TAddress, calculation = sp.TRecord(account = sp.TOption(sp.TAddress), borrowAmount = sp.TNat, cTokenModify = sp.TOption(sp.TAddress), redeemTokens = sp.TNat, sumBorrowPlusEffects = sp.TNat, sumCollateral = sp.TNat).layout((("account", ("borrowAmount", "cTokenModify")), ("redeemTokens", ("sumBorrowPlusEffects", "sumCollateral")))), closeFactorMantissa = sp.TNat, collaterals = sp.TBigMap(sp.TAddress, sp.TSet(sp.TAddress)), expScale = sp.TNat, halfExpScale = sp.TNat, liquidationIncentiveMantissa = sp.TNat, liquidityPeriodRelevance = sp.TNat, loans = sp.TBigMap(sp.TAddress, sp.TSet(sp.TAddress)), marketNameToAddress = sp.TMap(sp.TString, sp.TAddress), markets = sp.TBigMap(sp.TAddress, sp.TRecord(borrowCap = sp.TNat, borrowPaused = sp.TBool, collateralFactor = sp.TRecord(mantissa = sp.TNat).layout("mantissa"), isListed = sp.TBool, mintPaused = sp.TBool, name = sp.TString, price = sp.TRecord(mantissa = sp.TNat).layout("mantissa"), updateLevel = sp.TNat).layout(((("borrowCap", "borrowPaused"), ("collateralFactor", "isListed")), (("mintPaused", "name"), ("price", "updateLevel"))))), oracleAddress = sp.TAddress, pendingAdministrator = sp.TOption(sp.TAddress), pricePeriodRelevance = sp.TNat, transferPaused = sp.TBool).layout((((("account_liquidity", "activeOperations"), ("administrator", "calculation")), (("closeFactorMantissa", "collaterals"), ("expScale", "halfExpScale"))), ((("liquidationIncentiveMantissa", "liquidityPeriodRelevance"), ("loans", "marketNameToAddress")), (("markets", "oracleAddress"), ("pendingAdministrator", ("pricePeriodRelevance", "transferPaused")))))))
    self.init(account_liquidity = {},
              activeOperations = sp.set([]),
              administrator = sp.address('KT1UUZNzmqobhGXYvsbWUjnAAeWK3de2JG2q'),
              ...
              closeFactorMantissa = 1000000000000000000,
              collaterals = {},
              ...
              marketNameToAddress = {},
              markets = {},
              oracleAddress = sp.address('KT1MwuujtBodVQFm1Jk1KTGNc49wygqoLvpe'),
  ...
  def borrowAllowed(self, params):
    ...
    compute_Comptroller_632 = sp.local("compute_Comptroller_632", sp.as_nat(sp.level - self.data.account_liquidity[params.borrower].updateLevel, message = 'SUBTRACTION_UNDERFLOW'))
    sp.verify(compute_Comptroller_632.value < self.data.liquidityPeriodRelevance, 'CMPT_LIQUIDITY_OLD')
    sp.set_type(sp.level, sp.TNat)
    ...
    compute_Comptroller_167 = sp.local("compute_Comptroller_167", self.data.account_liquidity[params.borrower].liquidity - sp.to_int((self.data.markets[params.cToken].price.mantissa * params.borrowAmount) // self.data.expScale))
    sp.verify(compute_Comptroller_167.value >= 0, 'CMPT_REDEEMER_SHORTFALL')
  ...
  def calculateAccountAssetLiquidity(self, params):
    ...
    compute_Comptroller_384 = sp.local("compute_Comptroller_384", sp.record(mantissa = params.exchangeRateMantissa))
    sp.set_type(sp.level, sp.TNat)
    ...
    sp.set_type(compute_Comptroller_384.value, sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
    sp.set_type((((self.data.markets[sp.sender].price.mantissa * self.data.markets[sp.sender].collateralFactor.mantissa) // self.data.expScale) * compute_Comptroller_384.value.mantissa) // self.data.expScale, sp.TNat)
    compute_Comptroller_387 = sp.local("compute_Comptroller_387", sp.record(mantissa = (((self.data.markets[sp.sender].price.mantissa * self.data.markets[sp.sender].collateralFactor.mantissa) // self.data.expScale) * compute_Comptroller_384.value.mantissa) // self.data.expScale))
    sp.set_type(compute_Comptroller_387.value, sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
    sp.set_type(compute_Comptroller_387.value, sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
    sp.set_type(params.cTokenBalance, sp.TNat)
    sp.set_type(compute_Comptroller_387.value.mantissa * params.cTokenBalance, sp.TNat)
    sp.set_type(sp.record(mantissa = compute_Comptroller_387.value.mantissa * params.cTokenBalance), sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
    self.data.calculation.sumCollateral += (compute_Comptroller_387.value.mantissa * params.cTokenBalance) // self.data.expScale
    sp.set_type(self.data.markets[sp.sender].price, sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
    sp.set_type(self.data.markets[sp.sender].price, sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
    ...
    sp.if sp.some(sp.sender) == self.data.calculation.cTokenModify:
      ...
      sp.set_type(compute_Comptroller_387.value.mantissa * self.data.calculation.redeemTokens, sp.TNat)
      sp.set_type(sp.record(mantissa = compute_Comptroller_387.value.mantissa * self.data.calculation.redeemTokens), sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
      self.data.calculation.sumCollateral += (compute_Comptroller_387.value.mantissa * self.data.calculation.redeemTokens) // self.data.expScale
      sp.set_type(self.data.markets[sp.sender].price, sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
      sp.set_type(self.data.markets[sp.sender].price, sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
  ...
  def getHypoAccountLiquidity(self, params):
    ...
    sp.if self.data.collaterals.contains(params.data.account):
      sp.for asset in self.data.collaterals[params.data.account].elements():
    ...
    sp.if self.data.loans.contains(params.data.account):
      sp.for asset in self.data.loans[params.data.account].elements():
  ...
  def redeemAllowed(self, params):
    ...
    sp.verify((self.data.markets.contains(params.cToken)) & self.data.markets[params.cToken].isListed, 'CMPT_MARKET_NOT_LISTED')
    sp.if self.data.collaterals.contains(params.redeemer):
      ...
      compute_Comptroller_632 = sp.local("compute_Comptroller_632", sp.as_nat(sp.level - self.data.account_liquidity[params.redeemer].updateLevel, message = 'SUBTRACTION_UNDERFLOW'))
      sp.verify(compute_Comptroller_632.value < self.data.liquidityPeriodRelevance, 'CMPT_LIQUIDITY_OLD')
      sp.set_type(sp.level, sp.TNat)
      sp.set_type(self.data.markets[params.cToken].updateLevel, sp.TNat)
      compute_Comptroller_270 = sp.local("compute_Comptroller_270", sp.as_nat(sp.level - self.data.markets[params.cToken].updateLevel, message = 'SUBTRACTION_UNDERFLOW'))
      ...
      compute_Comptroller_167 = sp.local("compute_Comptroller_167", self.data.account_liquidity[params.redeemer].liquidity - sp.to_int((self.data.markets[params.cToken].price.mantissa * params.redeemAmount) // self.data.expScale))
      sp.verify(compute_Comptroller_167.value >= 0, 'CMPT_REDEEMER_SHORTFALL')
  ...
  def setAccountSnapAndExitMarket(self, params):
    ...
    sp.if self.data.collaterals.contains(params.account):
      ...
      compute_Comptroller_632 = sp.local("compute_Comptroller_632", sp.as_nat(sp.level - self.data.account_liquidity[params.account].updateLevel, message = 'SUBTRACTION_UNDERFLOW'))
      sp.verify(compute_Comptroller_632.value < self.data.liquidityPeriodRelevance, 'CMPT_LIQUIDITY_OLD')
      sp.set_type(sp.level, sp.TNat)
      sp.set_type(self.data.markets[sp.sender].updateLevel, sp.TNat)
      compute_Comptroller_270 = sp.local("compute_Comptroller_270", sp.as_nat(sp.level - self.data.markets[sp.sender].updateLevel, message = 'SUBTRACTION_UNDERFLOW'))
      ...
      compute_Comptroller_167 = sp.local("compute_Comptroller_167", self.data.account_liquidity[params.account].liquidity - sp.to_int((self.data.markets[sp.sender].price.mantissa * params.cTokenBalance) // self.data.expScale))
      sp.verify(compute_Comptroller_167.value >= 0, 'CMPT_REDEEMER_SHORTFALL')
  ...
  def supportMarket(self, params):
    ...
    self.data.markets[params.cToken] = sp.record(borrowCap = 0, borrowPaused = True, collateralFactor = sp.record(mantissa = 900000000000000000), isListed = True, mintPaused = True, name = params.name, price = sp.record(mantissa = 0), updateLevel = 0)
  ...
  def transferAllowed(self, params):
    ...
    sp.verify((self.data.markets.contains(params.cToken)) & self.data.markets[params.cToken].isListed, 'CMPT_MARKET_NOT_LISTED')
    sp.if self.data.collaterals.contains(params.src):
      ...
      compute_Comptroller_632 = sp.local("compute_Comptroller_632", sp.as_nat(sp.level - self.data.account_liquidity[params.src].updateLevel, message = 'SUBTRACTION_UNDERFLOW'))
      sp.verify(compute_Comptroller_632.value < self.data.liquidityPeriodRelevance, 'CMPT_LIQUIDITY_OLD')
      sp.set_type(sp.level, sp.TNat)
      sp.set_type(self.data.markets[params.cToken].updateLevel, sp.TNat)
      compute_Comptroller_270 = sp.local("compute_Comptroller_270", sp.as_nat(sp.level - self.data.markets[params.cToken].updateLevel, message = 'SUBTRACTION_UNDERFLOW'))
      ...
      compute_Comptroller_167 = sp.local("compute_Comptroller_167", self.data.account_liquidity[params.src].liquidity - sp.to_int((self.data.markets[params.cToken].price.mantissa * params.transferTokens) // self.data.expScale))
      sp.verify(compute_Comptroller_167.value >= 0, 'CMPT_REDEEMER_SHORTFALL')
  ...
  def updateAssetPrice(self, params):
    ...
    sp.if self.data.markets[params].updateLevel < sp.level:
      ...
      sp.transfer((self.data.markets[params].name + '-USD', sp.self_entry_point('setAssetPrice')), sp.tez(0), sp.contract(sp.TPair(sp.TString, sp.TContract(sp.TPair(sp.TString, sp.TPair(sp.TTimestamp, sp.TNat)))), self.data.oracleAddress, entry_point='get').open_some())