...
class Contract(sp.Contract):
  ...
  def accrueInterestInternal(self, params):
    ...
    sp.transfer(sp.record(borrows = self.data.totalBorrows, cash = self.data.totalSupply // 1000000, cb = params, reserves = self.data.totalReserves), sp.tez(0), sp.contract(sp.TRecord(borrows = sp.TNat, cash = sp.TNat, cb = sp.TContract(sp.TNat), reserves = sp.TNat).layout((("borrows", "cash"), ("cb", "reserves"))), self.data.interestRateModel, entry_point='getBorrowRate').open_some())
