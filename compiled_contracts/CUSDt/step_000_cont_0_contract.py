...
class Contract(sp.Contract):
  def __init__(self):
    ...
    self.init(accrualBlockNumber = 0,
              ...
              fa2_TokenAddress = sp.address('KT1XnTn74bUtxHfDtBmm2bGZAQfhPbvKWR8o'),
              ...
              ledger = {},
              metadata = {'' : sp.bytes('0x74657a6f732d73746f726167653a64617461'), 'data' : sp.bytes('')},
              ...
              tokenId = 0,
              token_metadata = {0 : sp.record(token_id = 0, token_info = {'decimals' : sp.bytes('0x36'), 'name' : sp.bytes('0x54657a46696e20496e7465726573742d42656172696e672055534420546574686572'), 'symbol' : sp.bytes('0xea9cb055534474')})},
              totalBorrows = 0,
              totalReserves = 0,
              totalSupply = 0)
  ...
  def accrueInterest(self, params):
    sp.set_type(params, sp.TUnit)
    compute_CFA2_47 = sp.local("compute_CFA2_47", (self.data.currentCash + self.data.totalSupply) + self.data.totalReserves)
    sp.if compute_CFA2_47.value > 0:
      self.data.activeOperations.add(13)
      sp.transfer(sp.record(callback = sp.self_entrypoint('setCash'), requests = sp.list([sp.record(owner = sp.self_address, token_id = self.data.tokenId)])), sp.tez(0), sp.contract(sp.TRecord(callback = sp.TContract(sp.TList(sp.TRecord(balance = sp.TNat, request = sp.TRecord(owner = sp.TAddress, token_id = sp.TNat).layout(("owner", "token_id"))).layout(("request", "balance")))), requests = sp.TList(sp.TRecord(owner = sp.TAddress, token_id = sp.TNat).layout(("owner", "token_id")))).layout(("requests", "callback")), self.data.fa2_TokenAddress, entrypoint='balance_of').open_some())
  ...
  def addReservesInternal(self, params):
    ...
    sp.transfer(sp.list([sp.record(from_ = params.originalSender, txs = sp.list([sp.record(amount = params.addAmount, to_ = sp.self_address, token_id = self.data.tokenId)]))]), sp.tez(0), sp.contract(sp.TList(sp.TRecord(from_ = sp.TAddress, txs = sp.TList(sp.TRecord(amount = sp.TNat, to_ = sp.TAddress, token_id = sp.TNat).layout(("to_", ("token_id", "amount"))))).layout(("from_", "txs"))), self.data.fa2_TokenAddress, entrypoint='transfer').open_some())
  ...
  def borrow(self, params):
    ...
    sp.verify(sp.level == self.data.accrualBlockNumber, 'CT_INTEREST_OLD')
    sp.transfer(sp.list([sp.record(from_ = sp.self_address, txs = sp.list([sp.record(amount = params, to_ = sp.sender, token_id = self.data.tokenId)]))]), sp.tez(0), sp.contract(sp.TList(sp.TRecord(from_ = sp.TAddress, txs = sp.TList(sp.TRecord(amount = sp.TNat, to_ = sp.TAddress, token_id = sp.TNat).layout(("to_", ("token_id", "amount"))))).layout(("from_", "txs"))), self.data.fa2_TokenAddress, entrypoint='transfer').open_some())
  ...
  def liquidateBorrow(self, params):
    ...
    sp.if self.data.borrows.contains(params.borrower):
      ...
      sp.if borrowSnapshot.value.principal > 0:
        borrowBalance.value = (borrowSnapshot.value.principal * self.data.borrowIndex) // borrowSnapshot.value.interestIndex
    sp.transfer(sp.list([sp.record(from_ = sp.sender, txs = sp.list([sp.record(amount = sp.min(borrowBalance.value, params.repayAmount), to_ = sp.self_address, token_id = self.data.tokenId)]))]), sp.tez(0), sp.contract(sp.TList(sp.TRecord(from_ = sp.TAddress, txs = sp.TList(sp.TRecord(amount = sp.TNat, to_ = sp.TAddress, token_id = sp.TNat).layout(("to_", ("token_id", "amount"))))).layout(("from_", "txs"))), self.data.fa2_TokenAddress, entrypoint='transfer').open_some())
  ...
  def mint(self, params):
    ...
    sp.verify(sp.level == self.data.accrualBlockNumber, 'CT_INTEREST_OLD')
    sp.transfer(sp.list([sp.record(from_ = sp.sender, txs = sp.list([sp.record(amount = params, to_ = sp.self_address, token_id = self.data.tokenId)]))]), sp.tez(0), sp.contract(sp.TList(sp.TRecord(from_ = sp.TAddress, txs = sp.TList(sp.TRecord(amount = sp.TNat, to_ = sp.TAddress, token_id = sp.TNat).layout(("to_", ("token_id", "amount"))))).layout(("from_", "txs"))), self.data.fa2_TokenAddress, entrypoint='transfer').open_some())
    excRate = sp.local("excRate", self.data.initialExchangeRateMantissa)
    sp.if self.data.totalSupply > 0:
      sp.set_type(self.data.currentCash, sp.TNat)
      sp.set_type(0, sp.TNat)
  ...
  def redeem(self, params):
    ...
    sp.if (redeem_amount.value > 0) & (redeem_tokens.value > 0):
      ...
      sp.transfer(sp.list([sp.record(from_ = sp.self_address, txs = sp.list([sp.record(amount = redeem_amount.value, to_ = sp.sender, token_id = self.data.tokenId)]))]), sp.tez(0), sp.contract(sp.TList(sp.TRecord(from_ = sp.TAddress, txs = sp.TList(sp.TRecord(amount = sp.TNat, to_ = sp.TAddress, token_id = sp.TNat).layout(("to_", ("token_id", "amount"))))).layout(("from_", "txs"))), self.data.fa2_TokenAddress, entrypoint='transfer').open_some())
  ...
  def reduceReservesInternal(self, params):
    ...
    sp.transfer(sp.list([sp.record(from_ = sp.self_address, txs = sp.list([sp.record(amount = params, to_ = self.data.administrator, token_id = self.data.tokenId)]))]), sp.tez(0), sp.contract(sp.TList(sp.TRecord(from_ = sp.TAddress, txs = sp.TList(sp.TRecord(amount = sp.TNat, to_ = sp.TAddress, token_id = sp.TNat).layout(("to_", ("token_id", "amount"))))).layout(("from_", "txs"))), self.data.fa2_TokenAddress, entrypoint='transfer').open_some())
  ...
  def repayBorrow(self, params):
    ...
    sp.if self.data.borrows.contains(sp.sender):
      ...
      sp.if borrowSnapshot.value.principal > 0:
        borrowBalance.value = (borrowSnapshot.value.principal * self.data.borrowIndex) // borrowSnapshot.value.interestIndex
    sp.transfer(sp.list([sp.record(from_ = sp.sender, txs = sp.list([sp.record(amount = sp.min(borrowBalance.value, params), to_ = sp.self_address, token_id = self.data.tokenId)]))]), sp.tez(0), sp.contract(sp.TList(sp.TRecord(from_ = sp.TAddress, txs = sp.TList(sp.TRecord(amount = sp.TNat, to_ = sp.TAddress, token_id = sp.TNat).layout(("to_", ("token_id", "amount"))))).layout(("from_", "txs"))), self.data.fa2_TokenAddress, entrypoint='transfer').open_some())
    sp.set_type(borrowBalance.value, sp.TNat)
