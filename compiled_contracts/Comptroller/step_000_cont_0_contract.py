...
class Contract(sp.Contract):
  ...
  def borrowAllowed(self, params):
    ...
    compute_Comptroller_803 = sp.local("compute_Comptroller_803", sp.as_nat(sp.level - self.data.account_liquidity[params.borrower].updateLevel, message = 'SUBTRACTION_UNDERFLOW'))
    sp.verify(compute_Comptroller_803.value == 0, 'CMPT_LIQUIDITY_OLD')
  ...
  def liquidateBorrowAllowed(self, params):
    ...
    sp.if self.data.collaterals.contains(params.borrower):
      sp.for asset in self.data.collaterals[params.borrower].elements():
        ...
        sp.set_type(compute_Comptroller_532.value, sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
        sp.set_type((((self.data.markets[asset].price.mantissa * self.data.markets[asset].collateralFactor.mantissa) // self.data.expScale) * compute_Comptroller_532.value.mantissa) // self.data.expScale, sp.TNat)
        compute_Comptroller_536 = sp.local("compute_Comptroller_536", sp.record(mantissa = (((self.data.markets[asset].price.mantissa * self.data.markets[asset].collateralFactor.mantissa) // self.data.expScale) * compute_Comptroller_532.value.mantissa) // self.data.expScale))
        ...
        sp.if (self.data.collaterals.contains(params.value.account)) & (self.data.collaterals[params.value.account].contains(asset)):
          sp.set_type(compute_Comptroller_536.value, sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
          sp.set_type(compute_Comptroller_536.value, sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
          sp.set_type(params.value.cTokenBalance, sp.TNat)
          sp.set_type(compute_Comptroller_536.value.mantissa * params.value.cTokenBalance, sp.TNat)
          sp.set_type(sp.record(mantissa = compute_Comptroller_536.value.mantissa * params.value.cTokenBalance), sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
          calc.value.sumCollateral += (compute_Comptroller_536.value.mantissa * params.value.cTokenBalance) // self.data.expScale
        sp.set_type(self.data.markets[asset].price, sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
        sp.set_type(self.data.markets[asset].price, sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
    ...
    compute_Comptroller_390 = sp.local("compute_Comptroller_390", calcLiquidity.value.sumCollateral - calcLiquidity.value.sumBorrowPlusEffects)
    liquidtiy = sp.local("liquidtiy", compute_Comptroller_390.value)
  ...
  def redeemAllowed(self, params):
    ...
    sp.if (self.data.collaterals.contains(params.redeemer)) & (self.data.collaterals[params.redeemer].contains(params.cToken)):
      sp.if (self.data.loans.contains(params.redeemer)) & (sp.len(self.data.loans[params.redeemer]) != 0):
        ...
        compute_Comptroller_803 = sp.local("compute_Comptroller_803", sp.as_nat(sp.level - self.data.account_liquidity[params.redeemer].updateLevel, message = 'SUBTRACTION_UNDERFLOW'))
        sp.verify(compute_Comptroller_803.value == 0, 'CMPT_LIQUIDITY_OLD')
        sp.verify(sp.level == self.data.markets[params.cToken].updateLevel, 'CMPT_UPDATE_PRICE')
        sp.verify(self.data.markets[params.cToken].price.mantissa > 0, 'CMPT_INVALID_PRICE')
        sp.set_type(self.data.markets[params.cToken].price, sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
  ...
  def setAccountSnapAndExitMarket(self, params):
    ...
    sp.if (self.data.collaterals.contains(params.account)) & (self.data.collaterals[params.account].contains(sp.sender)):
      sp.if (self.data.loans.contains(params.account)) & (sp.len(self.data.loans[params.account]) != 0):
        ...
        compute_Comptroller_803 = sp.local("compute_Comptroller_803", sp.as_nat(sp.level - self.data.account_liquidity[params.account].updateLevel, message = 'SUBTRACTION_UNDERFLOW'))
        sp.verify(compute_Comptroller_803.value == 0, 'CMPT_LIQUIDITY_OLD')
  ...
  def transferAllowed(self, params):
    ...
    sp.if (self.data.collaterals.contains(params.src)) & (self.data.collaterals[params.src].contains(params.cToken)):
      sp.if (self.data.loans.contains(params.src)) & (sp.len(self.data.loans[params.src]) != 0):
        ...
        compute_Comptroller_803 = sp.local("compute_Comptroller_803", sp.as_nat(sp.level - self.data.account_liquidity[params.src].updateLevel, message = 'SUBTRACTION_UNDERFLOW'))
        sp.verify(compute_Comptroller_803.value == 0, 'CMPT_LIQUIDITY_OLD')
        sp.verify(sp.level == self.data.markets[params.cToken].updateLevel, 'CMPT_UPDATE_PRICE')
        sp.verify(self.data.markets[params.cToken].price.mantissa > 0, 'CMPT_INVALID_PRICE')
        sp.set_type(self.data.markets[params.cToken].price, sp.TRecord(mantissa = sp.TNat).layout("mantissa"))
