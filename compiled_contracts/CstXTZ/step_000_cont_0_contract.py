...
class Contract(sp.Contract):
  def __init__(self):
    ...
    self.init(accrualBlockNumber = 0,
              ...
              administrator = sp.address('KT1QScMEtDpXSuj7z2if1EMSqaXaXPnWCxqv'),
              borrowIndex = 1000000000000000000,
              borrowRateMaxMantissa = 800000000000,
              borrowRatePerBlock = 0,
              borrows = {},
              comptroller = sp.address('KT1P6Lryn3ikbyf5jywWBBRP5fkztE5ZafGe'),
              currentCash = 0,
              expScale = 1000000000000000000,
              fa2_TokenAddress = sp.address('KT1KXKhkxDezoa8G3WvPtsrgNTs5ZQwhpYZN'),
              ...
              ledger = {},
              metadata = {'' : sp.bytes('0x74657a6f732d73746f726167653a64617461'), 'data' : sp.bytes('')},
              ...
              tokenId = 0,
              token_metadata = {0 : sp.record(token_id = 0, token_info = {'decimals' : sp.bytes('0x36'), 'name' : sp.bytes('0x54657a46696e20496e7465726573742d42656172696e6720737458545a'), 'symbol' : sp.bytes('0xea9cb0737458545a')})},
              totalBorrows = 0,
              totalReserves = 0,
              totalSupply = 0)
  ...
  def getCash(self, params):
    ...
    with __s7:
      sp.set_type(sp.fst(params), sp.TUnit)
      sp.result((self.data.currentCash, self.data.accrualBlockNumber))
    sp.set_type(sp.snd(params), sp.TContract(sp.TPair(sp.TNat, sp.TNat)))
  ...
  def liquidateBorrow(self, params):
    ...
    sp.transfer(sp.list([sp.record(from_ = sp.sender, txs = sp.list([sp.record(amount = repayAmount.value, to_ = sp.self_address, token_id = self.data.tokenId)]))]), sp.tez(0), sp.contract(sp.TList(sp.TRecord(from_ = sp.TAddress, txs = sp.TList(sp.TRecord(amount = sp.TNat, to_ = sp.TAddress, token_id = sp.TNat).layout(("to_", ("token_id", "amount"))))).layout(("from_", "txs"))), self.data.fa2_TokenAddress, entrypoint='transfer').open_some())
