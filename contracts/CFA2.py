...
class CFA2(CToken.CToken):
    def __init__(self, comptroller_, interestRateModel_, initialExchangeRateMantissa_, administrator_, fa2_TokenAddress_, tokenId_, **extra_storage):
    ...
    def updateCash(self):
        ...
        with sp.if_ (cashSum > 0): # prevent transaction failing if CToken is not listed in underlying fa2 yet
            self.activateOp(CToken.OP.CTokenOperations.GET_CASH)
            handle = sp.contract(TBalanceOfParams, self.data.fa2_TokenAddress, "balance_of").open_some()
            data = sp.record(
                    callback = sp.self_entry_point("setCash"),
                    requests = sp.list([
                        sp.record(owner=sp.self_address, token_id=self.data.tokenId)
                    ])
                )
    ...
    def setCash(self, params):
        ...
        sp.verify(sp.sender == self.data.fa2_TokenAddress, EC.CT_INVALID_CASH_SENDER)
        sp.verify(sp.len(params) == 1, EC.CT_INVALID_CASH_DATA)
        sp.for cash in params:
            sp.verify(cash.request.token_id == self.data.tokenId, EC.CT_INVALID_CASH_DATA)
            sp.verify(cash.request.owner == sp.self_address, EC.CT_INVALID_CASH_DATA)
            self.data.currentCash = cash.balance

    def doTransferIn(self, from_, amount):
        self.transferFA2(from_, sp.self_address, amount, self.data.fa2_TokenAddress, self.data.tokenId)

    def doTransferOut(self, to_, amount, isContract = False):
        self.transferFA2(sp.self_address, to_, amount, self.data.fa2_TokenAddress, self.data.tokenId)

    def verifySweepFA2(self, tokenAddress, id):
        isNotUnderlying = (tokenAddress != self.data.fa2_TokenAddress) | (id != self.data.tokenId)
