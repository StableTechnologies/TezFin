...
def test():
    ...
    multiplierPerBlock = int(18e10) # 0.00000018 
    baseRatePerBlock = int(84e10) # 0.00000084
    alpha1 = int(5e17) # 0.5
    alpha2 = expScale
    alpha3 = expScale * expScale

    c1 = IRM.AlphaInterestRateModel(scale_=expScale, multiplierPerBlock_=multiplierPerBlock, baseRatePerBlock_=baseRatePerBlock, alpha_=alpha1,admin_ = admin.address)
    scenario += c1
    
    c2 = IRM.AlphaInterestRateModel(scale_=expScale, multiplierPerBlock_=multiplierPerBlock, baseRatePerBlock_=baseRatePerBlock, alpha_=alpha2,admin_ = admin.address)
    scenario += c2
    
    c3 = IRM.AlphaInterestRateModel(scale_=expScale, multiplierPerBlock_=multiplierPerBlock, baseRatePerBlock_=baseRatePerBlock, alpha_=alpha3 ,admin_ = admin.address)
    scenario += c3
    ...
    c1.updateMultiplierPerBlock(0).run(sender = admin.address)
    scenario.verify(c1.data.multiplierPerBlock == 0)
    
    c1.updateAdmin(alice.address).run(sender = admin.address)
    ...
    scenario.h2("Test getBorrowRate")
    scenario.h3("With zero params, result baseRatePerBlock")
    scenario += c3.getBorrowRate(sp.record(cash=sp.nat(0), 
                                           borrows=sp.nat(0), 
                                           reserves=sp.nat(0), 
                                           cb=view_result.typed.targetNat))
    scenario.verify_equal(view_result.data.last, sp.some(baseRatePerBlock))
    scenario.h3("With zero borrows, result baseRatePerBlock")
    scenario += c3.getBorrowRate(sp.record(cash=sp.nat(20), 
                                           borrows=sp.nat(0),
                                           reserves=sp.nat(10), 
                                           cb=view_result.typed.targetNat))
    scenario.verify_equal(view_result.data.last, sp.some(baseRatePerBlock))
    ...
    scenario.verify_equal(view_result.data.last, sp.some(1020000000000))
    scenario.h3("Fail if reserves > (cash + borrows)")
    scenario += c3.getBorrowRate(sp.record(cash=sp.nat(10), 
                                           borrows=sp.nat(10), 
                                           reserves=sp.nat(30), 
                                           cb=view_result.typed.targetNat)).run(valid=False)
    scenario.h3("Fail if reserves = (cash + borrows)")
    scenario += c3.getBorrowRate(sp.record(cash=sp.nat(10), 
                                           ...
                                           cb=view_result.typed.targetNat)).run(valid=False)
    scenario.h3("Calculate BorrowRate 1")
    scenario += c3.getBorrowRate(sp.record(cash=sp.nat(3), 
                                           borrows=sp.nat(5), 
                                           reserves=sp.nat(0), 
                                           cb=view_result.typed.targetNat))
    scenario.verify_equal(view_result.data.last, sp.some(952500000000))
    scenario.h3("Calculate BorrowRate 2")
    scenario += c3.getBorrowRate(sp.record(cash=sp.nat(int(3e18)), 
                                           borrows=sp.nat(int(5e18)), 
                                           reserves=sp.nat(0), 
                                           cb=view_result.typed.targetNat))
    scenario.verify_equal(view_result.data.last, sp.some(952500000000))
    scenario.h3("Calculate BorrowRate 3")
    scenario += c3.getBorrowRate(sp.record(cash=sp.nat(int(5e18)), 
                                           borrows=sp.nat(int(3e18)), 
                                           reserves=sp.nat(0), 
                                           cb=view_result.typed.targetNat))
    scenario.verify_equal(view_result.data.last, sp.some(907500000000))
    scenario.h3("Calculate BorrowRate 4")
    scenario += c3.getBorrowRate(sp.record(cash=sp.nat(5), 
                                           borrows=sp.nat(5), 
                                           reserves=sp.nat(2), 
                                           cb=view_result.typed.targetNat))
    scenario.verify_equal(view_result.data.last, sp.some(952500000000))
    ...
    scenario.h2("Test getSupplyRate")
    scenario.h3("With zero params, result 0")
    scenario += c3.getSupplyRate(sp.record(cash=sp.nat(0),
                                           borrows=sp.nat(0),
                                           reserves=sp.nat(0), 
                                           reserveFactorMantissa=sp.nat(0), 
                                           cb=view_result.typed.targetNat))
    scenario.verify_equal(view_result.data.last, sp.some(0))
    scenario.h3("With reserveFactorMantissa = 1e18, result 0")
    scenario += c3.getSupplyRate(sp.record(cash=sp.nat(10),
                                           borrows=sp.nat(10),
                                           reserves=sp.nat(10), 
                                           reserveFactorMantissa=sp.nat(int(1e18)), 
                                           cb=view_result.typed.targetNat))
    ...
    scenario += c3.getSupplyRate(sp.record(cash=sp.nat(10), 
                                           ...
                                           cb=view_result.typed.targetNat)).run(valid=False)
    scenario.h3("Calculate SupplyRate 1")
    scenario += c3.getSupplyRate(sp.record(cash=sp.nat(500),
                                           borrows=sp.nat(300),
                                           reserves=sp.nat(0), 
                                           reserveFactorMantissa=sp.nat(int(1e17)), 
                                           cb=view_result.typed.targetNat))
    ...
    scenario.h3("Calculate SupplyRate 2")
    scenario += c3.getSupplyRate(sp.record(cash=sp.nat(int(5e18)),
                                           borrows=sp.nat(int(3e18)),
                                           reserves=sp.nat(0), 
                                           reserveFactorMantissa=sp.nat(int(1e17)), 
                                           cb=view_result.typed.targetNat))
    scenario.verify_equal(view_result.data.last, sp.some(306281250000))
    scenario.h3("Calculate SupplyRate 4")
    scenario += c3.getSupplyRate(sp.record(cash=sp.nat(30),
                                           borrows=sp.nat(50),
                                           reserves=sp.nat(0), 
                                           reserveFactorMantissa=sp.nat(int(1e17)), 
                                           cb=view_result.typed.targetNat))
    ...
    scenario.h3("Calculate SupplyRate 4")
    scenario += c3.getSupplyRate(sp.record(cash=sp.nat(70),
                                           borrows=sp.nat(30),
                                           reserves=sp.nat(20), 
                                           reserveFactorMantissa=sp.nat(int(1e17)), 
                                           cb=view_result.typed.targetNat))
    ...
    scenario.h2("Test getBorrowRate")
    scenario.h3("With zero params, result baseRatePerBlock")
    scenario += c2.getBorrowRate(sp.record(cash=sp.nat(0), 
                                           borrows=sp.nat(0), 
                                           reserves=sp.nat(0), 
                                           cb=view_result.typed.targetNat))
    scenario.verify_equal(view_result.data.last, sp.some(baseRatePerBlock))
    scenario.h3("With zero borrows, result baseRatePerBlock")
    scenario += c2.getBorrowRate(sp.record(cash=sp.nat(20), 
                                           borrows=sp.nat(0),
                                           reserves=sp.nat(10), 
                                           cb=view_result.typed.targetNat))
    scenario.verify_equal(view_result.data.last, sp.some(baseRatePerBlock))
    ...
    scenario.verify_equal(view_result.data.last, sp.some(1020000000000))
    scenario.h3("Fail if reserves > (cash + borrows)")
    scenario += c2.getBorrowRate(sp.record(cash=sp.nat(10), 
                                           borrows=sp.nat(10), 
                                           reserves=sp.nat(30), 
                                           cb=view_result.typed.targetNat)).run(valid=False)
    scenario.h3("Fail if reserves = (cash + borrows)")
    scenario += c2.getBorrowRate(sp.record(cash=sp.nat(10), 
                                           ...
                                           cb=view_result.typed.targetNat)).run(valid=False)
    scenario.h3("Calculate BorrowRate 1")
    scenario += c2.getBorrowRate(sp.record(cash=sp.nat(3), 
                                           borrows=sp.nat(5), 
                                           reserves=sp.nat(0), 
                                           cb=view_result.typed.targetNat))
    scenario.verify_equal(view_result.data.last, sp.some(978461538461))
    scenario.h3("Calculate BorrowRate 2")
    scenario += c2.getBorrowRate(sp.record(cash=sp.nat(int(3e18)), 
                                           borrows=sp.nat(int(5e18)), 
                                           reserves=sp.nat(0), 
                                           cb=view_result.typed.targetNat))
    scenario.verify_equal(view_result.data.last, sp.some(978461538461))
    scenario.h3("Calculate BorrowRate 3")
    scenario += c2.getBorrowRate(sp.record(cash=sp.nat(int(5e18)), 
                                           borrows=sp.nat(int(3e18)), 
                                           reserves=sp.nat(0), 
                                           cb=view_result.typed.targetNat))
    scenario.verify_equal(view_result.data.last, sp.some(938181818181))
    scenario.h3("Calculate BorrowRate 4")
    scenario += c2.getBorrowRate(sp.record(cash=sp.nat(5), 
                                           borrows=sp.nat(5), 
                                           reserves=sp.nat(2), 
                                           cb=view_result.typed.targetNat))
    scenario.verify_equal(view_result.data.last, sp.some(978461538461))
    ...
    scenario.h2("Test getSupplyRate")
    scenario.h3("With zero params, result 0")
    scenario += c2.getSupplyRate(sp.record(cash=sp.nat(0),
                                           borrows=sp.nat(0),
                                           reserves=sp.nat(0), 
                                           reserveFactorMantissa=sp.nat(0), 
                                           cb=view_result.typed.targetNat))
    scenario.verify_equal(view_result.data.last, sp.some(0))
    scenario.h3("With reserveFactorMantissa = 1e18, result 0")
    scenario += c2.getSupplyRate(sp.record(cash=sp.nat(10),
                                           borrows=sp.nat(10),
                                           reserves=sp.nat(10), 
                                           reserveFactorMantissa=sp.nat(int(1e18)), 
                                           cb=view_result.typed.targetNat))
    ...
    scenario += c2.getSupplyRate(sp.record(cash=sp.nat(10), 
                                           ...
                                           cb=view_result.typed.targetNat)).run(valid=False)
    scenario.h3("Calculate SupplyRate 1")
    scenario += c2.getSupplyRate(sp.record(cash=sp.nat(500),
                                           borrows=sp.nat(300),
                                           reserves=sp.nat(0), 
                                           reserveFactorMantissa=sp.nat(int(1e17)), 
                                           cb=view_result.typed.targetNat))
    scenario.verify_equal(view_result.data.last, sp.some(316636363636))
    scenario.h3("Calculate SupplyRate 2")
    scenario += c2.getSupplyRate(sp.record(cash=sp.nat(int(5e18)),
                                           borrows=sp.nat(int(3e18)),
                                           reserves=sp.nat(0), 
                                           reserveFactorMantissa=sp.nat(int(1e17)), 
                                           cb=view_result.typed.targetNat))
    scenario.verify_equal(view_result.data.last, sp.some(316636363636))
    scenario.h3("Calculate SupplyRate 4")
    scenario += c2.getSupplyRate(sp.record(cash=sp.nat(30),
                                           borrows=sp.nat(50),
                                           reserves=sp.nat(0), 
                                           reserveFactorMantissa=sp.nat(int(1e17)), 
                                           cb=view_result.typed.targetNat))
    scenario.verify_equal(view_result.data.last, sp.some(550384615384))
    scenario.h3("Calculate SupplyRate 4")
    scenario += c2.getSupplyRate(sp.record(cash=sp.nat(70),
                                           borrows=sp.nat(30),
                                           reserves=sp.nat(20), 
                                           reserveFactorMantissa=sp.nat(int(1e17)), 
                                           cb=view_result.typed.targetNat))
    ...
    scenario += c2.getSupplyRate(sp.record(cash=sp.nat(500),
                                           ...
                                           reserveFactorMantissa=sp.nat(int(2e17)), 
                                           cb=view_result.typed.targetNat))
    scenario.verify_equal(view_result.data.last, sp.some(281454545454))
