...
class TestCToken(CToken.CToken):
    def __init__(self, decimalsUnderlying_, comptroller_, interestRateModel_, initialExchangeRateMantissa_, administrator_, **extra_storage):
        CToken.CToken.__init__(self, decimalsUnderlying_, comptroller_, interestRateModel_, initialExchangeRateMantissa_, administrator_,
...
def test():
    ...
    c1 = TestCToken(decimalsUnderlying_=sp.nat(0),
    ...
    scenario += c1.mint(1000).run(sender=bob, level=bLevel.next())
    scenario.verify(c1.data.balances[bob.address].balance == sp.nat(1000 * ctoken_decimals))
    scenario.h3("Try mint internal")
    scenario += c1.mintInternal(sp.record(minter=alice.address, mintAmount=100)).run(sender=alice, level=bLevel.next(), valid=False)
    ...
    scenario += c1.borrow(10).run(sender=carl, level=bLevel.next())
    ...
    scenario += c1.borrow(10).run(sender=alice, level=bLevel.next())
    scenario.verify(c1.data.balances[alice.address].balance == sp.nat(100 * ctoken_decimals))
    scenario.verify(c1.data.balances[alice.address].accountBorrows.principal == sp.nat(10))
    scenario.h3("Try borrow internal")
    scenario += c1.borrowInternal(sp.record(borrower=alice.address, borrowAmount=100)).run(sender=alice, level=bLevel.next(), valid=False)
    scenario.h3("Try borrow with insufficient cash")
    scenario += c1.borrow(1100 * ctoken_decimals + 1).run(sender=carl, level=bLevel.next(), valid=False)
    scenario.h3("Try borrow in callback")
    scenario += c1.getCash(sp.pair(sp.unit, c1.typed.borrow)).run(sender=alice, level=bLevel.next(), valid=False)
    
    scenario.h2("Test Redeem")
    scenario.h3("Redeem allowed")
    scenario += c1.redeem(50 * ctoken_decimals).run(sender=alice, level=bLevel.next())
    ...
    scenario += c1.redeem(10 * ctoken_decimals).run(sender=alice, level=bLevel.next(), valid=False)
    scenario.h3("Redeem allowed again")
    scenario += cmpt.setRedeemAllowed(sp.bool(True))
    scenario += c1.redeem(10 * ctoken_decimals).run(sender=alice, level=bLevel.next())
    scenario.verify(c1.data.balances[alice.address].balance == sp.nat(40 * ctoken_decimals))
    scenario.h3("Try redeem internal")
    scenario += c1.redeemInternal(sp.record(redeemer=alice.address, redeemAmount=100, isUnderlying=False)).run(sender=alice, level=bLevel.next(), valid=False)
    scenario.h3("Try redeem with insufficient balance")
    scenario += c1.redeem(50 * ctoken_decimals).run(sender=alice, level=bLevel.next(), valid=False)
    scenario.h3("Redeem underlying")
    scenario += c1.redeemUnderlying(10).run(sender=alice, level=bLevel.next())
    ...
    scenario += c1.repayBorrow(1).run(sender=alice, level=bLevel.next())
    ...
    scenario += c1.repayBorrow(1).run(sender=alice, level=bLevel.next(), valid=False)
    ...
    scenario.h3("Try repay borrow internal")
    scenario += c1.repayBorrowInternal(sp.record(payer=alice.address, borrower=alice.address, repayAmount=1)).run(sender=alice, level=bLevel.next(), valid=False)
    scenario.h3("Repay borrow behalf")
    scenario += c1.repayBorrowBehalf(sp.record(borrower=alice.address, repayAmount=sp.nat(1))).run(sender=bob, level=bLevel.next())
    scenario.verify(c1.data.balances[alice.address].accountBorrows.principal == sp.nat(7))
    scenario.h3("Repay more than borrowed")
    scenario += c1.repayBorrow(1000).run(sender=alice, level=bLevel.next())
    scenario.verify(c1.data.balances[alice.address].accountBorrows.principal == sp.nat(0))
    scenario.h3("Try repayBorrow in callback")
    scenario += c1.getCash(sp.pair(sp.unit, c1.typed.redeem)).run(sender=alice, level=bLevel.next(), valid=False)
    
    scenario.h2("Test Seize")
    scenario += c1.setActiveOp(CToken.OP.CTokenOperations.LIQUIDATE).run(sender = admin, level = bLevel.next())
    scenario.h3("Called not by comptroller")
    scenario += c1.seize(sp.record(liquidator=bob.address, borrower=alice.address, seizeTokens=1000)).run(sender=bob, level=bLevel.next(), valid=False)
    scenario.h3("Called by comptroller")
    scenario += cmpt.testSeize(sp.record(cToken=c1.address,
                                         data=sp.record(liquidator=bob.address, borrower=alice.address, seizeTokens=188680))).run(sender=bob, level=bLevel.next())
    scenario.verify(c1.data.balances[alice.address].balance == sp.nat(30000000))
    scenario.verify(c1.data.balances[bob.address].balance == sp.nat(1000188680))
    scenario.h3("Same liquidator and borrower")
    scenario += c1.setActiveOp(CToken.OP.CTokenOperations.LIQUIDATE).run(sender = admin, level = bLevel.next())
    scenario += cmpt.testSeize(sp.record(cToken=c1.address,
                                                data=sp.record(liquidator=alice.address, borrower=alice.address, seizeTokens=1000))).run(sender=alice, level=bLevel.next(), valid=False)
    scenario.h3("Insufficient borrower balance")
    scenario += cmpt.testSeize(sp.record(cToken=c1.address,
                                         data=sp.record(liquidator=alice.address, borrower=bob.address, seizeTokens=10000000000))).run(sender=alice, level=bLevel.next(), valid=False)
    scenario += c1.hardResetOp().run(sender = admin, level = bLevel.next())

    scenario.h2("Test liquidateBorrow")
    scenario.h3("Called not by comptroller")
    scenario.h3("Liquidate borrow allowed")
    scenario += c1.liquidateBorrow(sp.record(borrower=carl.address, cTokenCollateral=collateralToken, repayAmount=sp.nat(1))).run(sender=alice, level=bLevel.next())
    scenario.verify(c1.data.balances[carl.address].accountBorrows.principal == sp.nat(9))
    scenario.h3("Liquidate borrow not allowed")
    scenario += cmpt.setLiquidateBorrowAllowed(sp.bool(False))
    scenario += c1.liquidateBorrow(sp.record(borrower=carl.address, cTokenCollateral=collateralToken, repayAmount=sp.nat(1))).run(sender=alice, level=bLevel.next(), valid=False)
    scenario.h3("Liquidate borrow allowed again")
    scenario += cmpt.setLiquidateBorrowAllowed(sp.bool(True))
    scenario += c1.liquidateBorrow(sp.record(borrower=carl.address, cTokenCollateral=collateralToken, repayAmount=sp.nat(1))).run(sender=alice, level=bLevel.next())
    scenario.verify(c1.data.balances[carl.address].accountBorrows.principal == sp.nat(8))
    scenario.h3("Liquidator is the same as borrower")
    scenario += c1.liquidateBorrow(sp.record(borrower=carl.address, cTokenCollateral=collateralToken, repayAmount=sp.nat(1))).run(sender=carl, level=bLevel.next(), valid=False)
    scenario.h3("Repay amount is zero")
    scenario += c1.liquidateBorrow(sp.record(borrower=carl.address, cTokenCollateral=collateralToken, repayAmount=sp.nat(0))).run(sender=alice, level=bLevel.next(), valid=False)
    scenario.h3("Liquidate more than borrowed")
    scenario += c1.liquidateBorrow(sp.record(borrower=carl.address, cTokenCollateral=collateralToken, repayAmount=sp.nat(100))).run(sender=alice, level=bLevel.next())
    scenario.verify(c1.data.balances[carl.address].accountBorrows.principal == sp.nat(0))
    scenario.h3("Try liquidate borrow internal")
    scenario += c1.liquidateBorrowInternal(sp.record(liquidator=alice.address,
                                                     borrower=carl.address, 
                                                     cTokenCollateral=collateralToken, 
                                                     repayAmount=sp.nat(10))).run(sender=alice, level=bLevel.next(), valid=False)
    scenario.h3("Try liquidate seize tokens internal")
    scenario += c1.liquidateSeizeTokens(sp.record(liquidator=alice.address,
                                                  borrower=carl.address, 
                                                  cTokenCollateral=collateralToken, 
                                                  oldPrincipal=sp.nat(10))).run(sender=alice, level=bLevel.next(), valid=False)
    ...
    CheckAdminRequirement(scenario, "set pending governance", bLevel, admin, alice, c1.setPendingGovernance, bob.address)

    scenario.h3("Accept governance")
    scenario.h4("Not pending admin attemps to accept governance")
    scenario += c1.acceptGovernance(sp.unit).run(sender=admin, level=bLevel.next(), valid=False)
    scenario.h4("Pending admin attemps to accept governance")
    scenario += c1.acceptGovernance(sp.unit).run(sender=bob, level=bLevel.next())
    scenario.verify(c1.data.administrator == bob.address)
    scenario.verify(c1.data.pendingAdministrator == c1.undefinedAddress())
    ...
    CheckAdminRequirement(scenario, "remove pending governance", bLevel, admin, alice, c1.removePendingGovernance, sp.unit)
    scenario.verify(c1.data.pendingAdministrator == c1.undefinedAddress())
 
    scenario.h3("Set comptroller")
    CheckAdminRequirement(scenario, "set comptroller", bLevel, admin, alice, c1.setComptroller, cmpt.address)
    scenario.verify(c1.data.comptroller == cmpt.address)

    scenario.h3("Set interest rate model")
    CheckAdminRequirement(scenario, "set interest rate model", bLevel, admin, alice, c1.setInterestRateModel, irm.address)
    scenario.verify(c1.data.interestRateModel == irm.address)

    scenario.h3("Set reserve factor")
    reserveFactorArg = sp.nat(1)
    CheckAdminRequirement(scenario, "Set reserve factor", bLevel, admin, alice, c1.setReserveFactor, reserveFactorArg)
    ...
    amountArg = sp.nat(5)
    CheckAdminRequirement(scenario, "reduce reserves", bLevel, admin, alice, c1.reduceReserves, amountArg)
    ...
    CheckAdminRequirement(scenario, "hardResetOp", bLevel, admin, alice, c1.hardResetOp, sp.unit)
    ...
    scenario.verify_equal(view_result.data.last, sp.some(29))
    scenario.h3("Mint new tokens and check balance")
    scenario += c1.mint(100).run(sender=alice, level=bLevel.next())
    scenario += c1.getBalanceOfUnderlying(sp.pair(alice.address, view_result.typed.targetNat)).run(sender = alice, level = bLevel.next())
    scenario.verify_equal(view_result.data.last, sp.some(129))
    scenario.h3("Try with nonexistent account")
    scenario += c1.getBalanceOfUnderlying(sp.pair(collateralToken, view_result.typed.targetNat)).run(sender = alice, level = bLevel.next(), valid=False)

    scenario.h2("Test getAccountSnapshot")
    scenario += c1.updateAccountSnapshot(alice.address).run(sender = alice, level = bLevel.next())
    scenario.verify(c1.data.accCTokenBalance == sp.nat(130506212))
    scenario.verify(c1.data.accBorrowBalance == sp.nat(0))
    scenario.verify(c1.data.accExchangeRateMantissa == sp.nat(994963369835))    

    scenario.h2("Test updateBorrowRatePerBlock")
    scenario.h3("Updated")
    borrowRate = sp.nat(90000000000)
    scenario += irm.setBorrowRate(borrowRate)
    scenario += c1.updateBorrowRatePerBlock().run(sender = alice, level = bLevel.next())
    scenario.verify(c1.data.borrowRatePerBlock == borrowRate)
    scenario.h3("Try internal")
    scenario += c1.updateBorrowRatePerBlockInternal().run(sender = alice, level = bLevel.next(), valid=False)
    scenario.h3("Try set directly")
    scenario += c1.setBorrowRatePerBlock(borrowRate).run(sender = alice, level = bLevel.next(), valid=False)

    scenario.h2("Test updateSupplyRatePerBlock")
    scenario.h3("Updated")
    supplyRate = sp.nat(80000000000)
    scenario += irm.setSupplyRate(supplyRate)
    scenario += c1.updateSupplyRatePerBlock().run(sender = alice, level = bLevel.next())
    scenario.verify(c1.data.supplyRatePerBlock == supplyRate)
    scenario.h3("Try internal")
    scenario += c1.updateSupplyRatePerBlockInternal().run(sender = alice, level = bLevel.next(), valid=False)
    scenario.h3("Try set directly")
    scenario += c1.setSupplyRatePerBlock(supplyRate).run(sender = alice, level = bLevel.next(), valid=False)


# Helpers
def CheckAdminRequirement(scenario, actionText, bLevel, adminAccount, nonAdminAccount, callableObj, *args):
    scenario.h4(f'Common user attempts to {actionText}')
    scenario += callableObj(*args).run(sender = nonAdminAccount, level = bLevel.next(), valid = False)

    scenario.h4(f'Admin attempts to {actionText}')
    scenario += callableObj(*args).run(sender = adminAccount, level = bLevel.next())
