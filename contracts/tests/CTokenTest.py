...
def test():
    ...
    scenario += c1.updateBorrowRateInternal().run(sender = alice, level = bLevel.next(), valid=False)
    ...
    scenario.h3("Try internal")
    scenario += c1.updateSupplyRateInternal().run(sender = alice, level = bLevel.next(), valid=False)
    scenario.h3("Try set directly")
