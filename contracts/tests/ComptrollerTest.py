...
TestAdminFunctionality = sp.io.import_script_from_url(
    "file:contracts/tests/utils/TestAdminFunctionality.py")
...
def test():
    ...
    scenario += oracle
    cmpt = ComptrollerTest(administrator_=admin.address,
                           oracleAddress_=oracle.address)
    scenario += cmpt
    ...
    marketsList = [listedMarket, notListedMarket,
                   listedMarketWithoutAccountMembership, cTokenMock.address]
    ...
    scenario.verify(
        cmpt.data.markets[listedMarket].mintPaused == sp.bool(True))
    ...
    scenario.verify(
        cmpt.data.markets[listedMarket].mintPaused == sp.bool(False))
    testPauseFunctionsOnMarkets(scenario, "Set mint paused", bLevel,
                                admin, cmpt.setMintPaused, notListedMarket, listedMarket)
    ...
    TestAdminFunctionality.checkAdminRequirementH4(scenario, "set borrow paused True", bLevel, admin, alice, cmpt.setBorrowPaused,
                                                   sp.record(cToken=listedMarket, state=sp.bool(True)))
    scenario.verify(
        cmpt.data.markets[listedMarket].borrowPaused == sp.bool(True))
    TestAdminFunctionality.checkAdminRequirementH4(scenario, "set borrow paused False", bLevel, admin, alice, cmpt.setBorrowPaused,
                                                   sp.record(cToken=listedMarket, state=sp.bool(False)))
    scenario.verify(
        cmpt.data.markets[listedMarket].borrowPaused == sp.bool(False))
    testPauseFunctionsOnMarkets(scenario, "Set borrow paused", bLevel,
                                admin, cmpt.setBorrowPaused, notListedMarket, listedMarket)

    scenario.h3("Set transfer paused")
    TestAdminFunctionality.checkAdminRequirementH4(
        scenario, "set transfer paused True", bLevel, admin, alice, cmpt.setTransferPaused, sp.bool(True))
    scenario.verify(cmpt.data.transferPaused == sp.bool(True))
    TestAdminFunctionality.checkAdminRequirementH4(
        scenario, "set transfer paused False", bLevel, admin, alice, cmpt.setTransferPaused, sp.bool(False))
    scenario.verify(cmpt.data.transferPaused == sp.bool(False))

    scenario.h2("Test allowed functionality")
    scenario.h3("Mint allowed")

    def minterArgLambda(market): return sp.record(
        cToken=market, minter=alice.address, mintAmount=sp.nat(100))
    scenario.h4("on the listed market")
    scenario += cmpt.mintAllowed(minterArgLambda(listedMarket)
                                 ).run(sender=alice, level=bLevel.next(), valid=True)
    scenario.h4("on the not listed market")
    scenario += cmpt.mintAllowed(minterArgLambda(notListedMarket)
                                 ).run(sender=alice, level=bLevel.next(), valid=False)
    scenario.h4("mint is paused")
    scenario += cmpt.setMintPaused(sp.record(cToken=listedMarket,
                                   state=sp.bool(True))).run(sender=admin, level=bLevel.next())
    scenario += cmpt.mintAllowed(minterArgLambda(listedMarket)
                                 ).run(sender=alice, level=bLevel.next(), valid=False)
    scenario.h4("mint is not paused")
    scenario += cmpt.setMintPaused(sp.record(cToken=listedMarket,
                                   state=sp.bool(False))).run(sender=admin, level=bLevel.next())
    scenario += cmpt.mintAllowed(minterArgLambda(listedMarket)
                                 ).run(sender=alice, level=bLevel.next())

    scenario.h3("Redeem allowed")
    cmpt.addToLoansExternal(
        sp.pair(alice.address, sp.set([cTokenMock.address])))

    def redeemArgLambda(market): return sp.record(
        cToken=market, redeemer=alice.address, redeemAmount=sp.nat(10*1000000000000000000))
    scenario.h4("on the listed market, without updated price")
    scenario += cmpt.redeemAllowed(redeemArgLambda(listedMarket)
                                   ).run(sender=alice, level=bLevel.next(), valid=False)
    scenario.h4(
    ...
    scenario.h4("on the not listed market")
    scenario += cmpt.redeemAllowed(redeemArgLambda(notListedMarket)).run(
        sender=alice, level=bLevel.current(), valid=False)
    scenario.h4("with insufficient liquidity")
    cTokenMock.setAccountSnapshot(sp.record(account=alice.address, cTokenBalance=sp.nat(
        0), borrowBalance=sp.nat(100), exchangeRateMantissa=exchRate)).run(level=bLevel.current())
    scenario += cmpt.updateAccountLiquidityWithView(
        alice.address).run(sender=alice, level=bLevel.next())
    scenario.show(cmpt.data.collaterals)
    scenario.show(alice.address)
    scenario += cmpt.redeemAllowed(redeemArgLambda(listedMarket)
                                   ).run(sender=alice, level=bLevel.current(), valid=False)
    scenario.h4("without insufficient liquidity")
    cTokenMock.setAccountSnapshot(sp.record(account=alice.address, cTokenBalance=sp.nat(
        100*1000000000000000000), borrowBalance=sp.nat(0), exchangeRateMantissa=exchRate)).run(level=bLevel.current())
    scenario += cmpt.updateAccountLiquidityWithView(
        alice.address).run(sender=alice, level=bLevel.next())
    scenario += cmpt.redeemAllowed(redeemArgLambda(listedMarket)
                                   ).run(sender=alice, level=bLevel.current())
    scenario.h4("invalid after price was not updated for 5 blocks")
    scenario += cmpt.redeemAllowed(redeemArgLambda(listedMarket)
                                   ).run(sender=alice, level=bLevel.add(5), valid=False)

    scenario.h3("Borrow allowed")

    def borrowArgLambda(market): return sp.record(
        cToken=market, borrower=alice.address, borrowAmount=sp.nat(100*1000000000000000000))
    scenario.h4("on the listed market, without updated price")
    scenario += cmpt.borrowAllowed(borrowArgLambda(listedMarket)
                                   ).run(sender=alice, level=bLevel.next(), valid=False)
    scenario.h4(
        "on the listed market, with updated price, without updated liquidity")
    updateAssetsPrices(scenario, cmpt, bLevel, marketsList)
    ...
    scenario += cmpt.updateAccountLiquidityWithView(
        alice.address).run(sender=alice, level=bLevel.next())
    scenario += cmpt.borrowAllowed(borrowArgLambda(listedMarket)
                                   ).run(sender=alice, level=bLevel.current(), valid=True)
    scenario.h4("on the not listed market")
    scenario += cmpt.borrowAllowed(borrowArgLambda(notListedMarket)).run(
        sender=alice, level=bLevel.current(), valid=False)
    scenario.h4("with insufficient liquidity")
    cTokenMock.setAccountSnapshot(sp.record(account=alice.address, cTokenBalance=sp.nat(
        0), borrowBalance=sp.nat(100), exchangeRateMantissa=exchRate)).run(level=bLevel.current())
    ...
    scenario += cmpt.setBorrowPaused(sp.record(cToken=listedMarket,
                                     state=sp.bool(True))).run(sender=admin, level=bLevel.current())
    scenario += cmpt.updateAccountLiquidityWithView(
        alice.address).run(sender=alice, level=bLevel.next())
    scenario += cmpt.borrowAllowed(borrowArgLambda(listedMarket)
                                   ).run(sender=alice, level=bLevel.current(), valid=False)
    ...
    scenario += cmpt.setBorrowPaused(sp.record(cToken=listedMarket,
                                     state=sp.bool(False))).run(sender=admin, level=bLevel.current())
    scenario += cmpt.borrowAllowed(borrowArgLambda(listedMarket)
                                   ).run(sender=alice, level=bLevel.current())
    scenario.h4("alice calls borrowAllowed if borrower not in market")
    scenario += cmpt.updateAccountLiquidityWithView(
        notMember.address).run(sender=alice, level=bLevel.next())
    ...
    def repayBorrowArgLambda(market): return sp.record(
        cToken=market, payer=admin.address, borrower=alice.address, repayAmount=sp.nat(100))
    scenario.h4("on the listed market")
    scenario += cmpt.repayBorrowAllowed(repayBorrowArgLambda(
        listedMarket)).run(sender=alice, level=bLevel.next())
    scenario.h4("on the not listed market")
    scenario += cmpt.repayBorrowAllowed(repayBorrowArgLambda(notListedMarket)).run(
        sender=alice, level=bLevel.next(), valid=False)

    scenario.h3("Transfer allowed")
    def transferArgLambda(market): return sp.record(
        cToken=market, src=alice.address, dst=admin.address, transferTokens=sp.nat(100))
    scenario.h4("redeem is allowed, without updated price")
    scenario += cmpt.transferAllowed(transferArgLambda(listedMarket)).run(
        sender=alice, level=bLevel.next(), valid=False)
    scenario.h4(
    ...
    scenario += cmpt.updateAccountLiquidityWithView(
        alice.address).run(sender=alice, level=bLevel.next())
    scenario += cmpt.transferAllowed(transferArgLambda(listedMarket)
                                     ).run(sender=alice, level=bLevel.current())
    scenario.h4("redeem is not allowed")
    scenario += cmpt.transferAllowed(transferArgLambda(notListedMarket)).run(
        sender=alice, level=bLevel.current(), valid=False)
    scenario.h4("transfer is paused")
    scenario += cmpt.setTransferPaused(sp.bool(True)
                                       ).run(sender=admin, level=bLevel.current())
    scenario += cmpt.updateAccountLiquidityWithView(
        alice.address).run(sender=alice, level=bLevel.next())
    scenario += cmpt.transferAllowed(transferArgLambda(listedMarket)).run(
        sender=alice, level=bLevel.current(), valid=False)
    scenario.h4("transfer is not paused")
    scenario += cmpt.setTransferPaused(sp.bool(False)
                                       ).run(sender=admin, level=bLevel.current())
    scenario += cmpt.transferAllowed(transferArgLambda(listedMarket)
                                     ).run(sender=alice, level=bLevel.current())
    scenario.h4("invalid after price was not updated for 5 blocks")
    scenario += cmpt.transferAllowed(transferArgLambda(listedMarket)).run(
        sender=alice, level=bLevel.add(5), valid=False)

    scenario.h3("Exit market")
    scenario.h4("The sender hasn't borrow balance, asset price was not updated")
    scenario += cTokenMock.setAccountSnapshot(sp.record(account=alice.address, cTokenBalance=sp.nat(
        10), borrowBalance=sp.nat(0), exchangeRateMantissa=exchRate)).run(level=bLevel.next())
    cmpt.exitMarket(cTokenMock.address).run(
        sender=alice, level=bLevel.next(), valid=False)
    scenario.h4(
    ...
    scenario += cmpt.updateAccountLiquidityWithView(
        alice.address).run(sender=alice, level=bLevel.next())
    scenario.verify(cmpt.data.collaterals[alice.address].contains(
        cTokenMock.address))  # account membership should exist before
    cmpt.exitMarket(cTokenMock.address).run(
        sender=alice, level=bLevel.current())
    scenario.verify((~ cmpt.data.collaterals[alice.address].contains(
        cTokenMock.address)))  # account membership must be removed
    scenario.h4("The sender has borrow balance")
    scenario += cTokenMock.setAccountSnapshot(sp.record(account=alice.address, cTokenBalance=sp.nat(
        10), borrowBalance=sp.nat(100), exchangeRateMantissa=exchRate))
    cmpt.exitMarket(cTokenMock.address).run(
        sender=alice, level=bLevel.current(), valid=False)
    ...
    scenario.verify_equal(
        cmpt.data.markets[listedMarket].price.mantissa, sp.nat(int(2e18)))
    scenario.verify_equal(
        cmpt.data.markets[listedMarket].updateLevel, bLevel.current())
    scenario.h3("Try to update price at the same level")
    oracle.setPrice(1)
    scenario += cmpt.updateAllAssetPricesWithView().run(sender=bob,
                                                        level=bLevel.current())
    scenario.verify_equal(
        cmpt.data.markets[listedMarket].price.mantissa, sp.nat(int(2e18)))

    scenario.h2("Test account liquidity")
    cmpt.enterMarkets(sp.list([cTokenMock.address])).run(
        sender=bob, level=bLevel.next())
    scenario.h3("Get current liquidity, without updated price")
    cTokenMock.setAccountSnapshot(sp.record(account=bob.address, cTokenBalance=sp.nat(
        10), borrowBalance=sp.nat(100), exchangeRateMantissa=exchRate))
    ...
    scenario.verify(cmpt.data.liquidationIncentiveMantissa ==
                    liquidationIncentiveMantissa)

    scenario.h3("Set collateral factor")
    collateralFactor = sp.record(
        cToken=listedMarket, newCollateralFactor=sp.nat(2))
    TestAdminFunctionality.checkAdminRequirementH4(scenario, "set collateral factor", bLevel, admin, alice, cmpt.setCollateralFactor,
                                                   collateralFactor)
    scenario.verify(
        cmpt.data.markets[collateralFactor.cToken].collateralFactor.mantissa == collateralFactor.newCollateralFactor)
    scenario.h4("Not listed market")
    notListedMarket = sp.test_account(
        "[setCollateralFactor] not listed market").address
    collateralFactor = sp.record(
        cToken=notListedMarket, newCollateralFactor=sp.nat(2))
    scenario += cmpt.setCollateralFactor(collateralFactor).run(
    ...
    supportMarketParams = sp.record(cToken=newMarket, name=sp.string(
        "market"), priceExp=1000000000000000000)
    TestAdminFunctionality.checkAdminRequirementH4(scenario, "support market", bLevel, admin, alice, cmpt.supportMarket,
                                                   supportMarketParams)
    scenario.verify(cmpt.data.markets.contains(newMarket) &
                    cmpt.data.markets[newMarket].isListed)
    scenario.verify(cmpt.data.marketNameToAddress.contains("market-USD"))
    scenario.h4("Already listed market")
    cmpt.supportMarket(supportMarketParams).run(
        sender=admin, level=bLevel.next(), valid=False)
    ...
    scenario.h4("Not listed market")
    notListedMarket = sp.test_account(
        "[disableMarket] not listed market").address
    cmpt.disableMarket(notListedMarket).run(
        sender=admin, level=bLevel.next(), valid=False)
    ...
    TestAdminFunctionality.checkAdminRequirementH4(scenario, "set market borrow cap", bLevel, admin, alice, cmpt.setMarketBorrowCap,
                                                   borrowCapRecord)
    scenario.verify(
        cmpt.data.markets[borrowCapRecord.cToken].borrowCap == borrowCapRecord.newBorrowCap)
    scenario.h4("Non-existant market")
    notExistantMarket = sp.test_account(
        "[setMarketBorrowCap] non-existant market").address
    borrowCapRecord = sp.record(
        cToken=notExistantMarket, newBorrowCap=sp.nat(2))
    scenario += cmpt.setMarketBorrowCap(borrowCapRecord).run(
        sender=admin, level=bLevel.next(), valid=False)

    scenario.h3("Pending governance")
    pendingGovernance = sp.test_account("[governance] pending governance")
    TestAdminFunctionality.checkAdminRequirementH4(
        scenario, "set pending governance", bLevel, admin, alice, cmpt.setPendingGovernance, pendingGovernance.address)
    scenario.verify(cmpt.data.pendingAdministrator.open_some()
                    == pendingGovernance.address)

    scenario.h3("Accept governance")
    scenario.h4("Not pending admin attempts to accept governance")
    scenario += cmpt.acceptGovernance(sp.unit).run(sender=admin,
                                                   level=bLevel.next(), valid=False)
    scenario.h4("Pending admin attempts to accept governance")
    scenario += cmpt.acceptGovernance(sp.unit).run(
        sender=pendingGovernance, level=bLevel.next())
    ...
    scenario += cmpt.setPendingGovernance(admin.address).run(
        sender=pendingGovernance, level=bLevel.next())
    scenario += cmpt.acceptGovernance(sp.unit).run(sender=admin,
                                                   level=bLevel.next())
...
def initMarkets(scenario, bLevel, marketPairs, testCMPT):
    for marketPair in marketPairs:
        scenario += testCMPT.addMarket(marketPair).run(
            level=bLevel.next(), show=False)
...
def testPauseFunctionsOnMarkets(scenario, actionText, bLevel, sender, callableObj, notListedMarket, listedMarket, isValidOnNotListedMarket=False, isValidOnListedMarket=True):
    ...
    scenario += callableObj(sp.record(cToken=listedMarket, state=sp.bool(False))).run(
        sender=sender, level=bLevel.next(), valid=isValidOnListedMarket)
    scenario.h4(f'{actionText} on not listed market')
    scenario += callableObj(sp.record(cToken=notListedMarket, state=sp.bool(False))).run(
        sender=sender, level=bLevel.next(), valid=isValidOnNotListedMarket)
