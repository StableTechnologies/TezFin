def checkAdminRequirementH4(scenario, actionText, bLevel, adminAccount, nonAdminAccount, callableObj, arg, **runOptions):
    ...
    scenario += callableObj(arg).run(sender = nonAdminAccount, level = bLevel.next(), valid = False, **runOptions)

    scenario.h4(f'Admin attempts to {actionText}')
    scenario += callableObj(arg).run(sender = adminAccount, level = bLevel.next(), **runOptions)
