...
const DashboardModal = (props) => {
  ...
  const { address, underlyingBalances, isKeyRevealed } = useSelector(
    (state) => state.addWallet.account,
  );
  const { totalCollateral } = useSelector(
    (state) => state.supplyComposition.supplyComposition,
  );
  const { borrowing, borrowLimit } = useSelector(
    (state) => state.borrowComposition.borrowComposition,
  );
      ...
      <Dialog open={open} onClose={close} className={classes.root}>
        ...
        {!collateralize && (
          <DialogTitle>
            <div>
              ...
              >
                <Typography
                  className={`${classes.modalText} ${classes.imgTitle}`}
                >
                  {tokenDetails.walletBalance
                    ? nFormatter(
                        decimalify(
        ...
        {(!visibility || collateralize) && (
          <Box className={classes.fTokenImgCon}>
            <img
              src={tokenDetails.fLogo}
              alt="logo"
              className={classes.fTokenImg}
            />
          </Box>
        )}
        {visibility && !collateralize ? (
          <Box className={classes.formFieldCon}>
            <form className={classes.form}>
              <TextField
                ...
                onInput={(e) =>
                  setTokenValue(e.target.value.replace(/[^0-9.]/g, ""))
                }
                onChange={(e) =>
                  setAmount(
                    undecimalify(e.target.value, decimals[tokenDetails.title]),
                  )
                }
                value={tokenValue}
                inputProps={{
          ...
          <Box
            className={`${classes.contentBoxOne} ${extraPadding} ${classes.borderTop0}`}
          >
            <Typography className={`${classes.headerText} ${classes.margin0}`}>
              {" "}
              {headerText}{" "}
            </Typography>
          </Box>
            ...
            <Box className={classes.contentBoxOne}>
              <Grid container justifyContent="space-between">
                ...
                {mainModal ? (
                  ...
                  >
                    {tokenDetails.rate > 0
                      ? roundValue(decimalify(tokenDetails.rate, 18))
                      : "0"}
                    {"%"}
                  </Grid>
                )}
